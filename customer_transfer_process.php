...
<?php
     ...
     $sender_id=$_SESSION["login_id"];// sender Account_no
     $reciever_id=$_REQUEST['transfer'];// receiver Account_no
     
     //select last transaction id in reciever's passbook.
     include_once 'dbconnect.php';
     $sql="SELECT MAX(transactionid) from `".$reciever_id."`";
     $result=mysql_query($sql) or die(mysql_error());
     $rws=  mysql_fetch_array($result);
     $r_last_tid=$rws[0];
    
    //select the details in the last row of reciever's passbook.
    $sql="SELECT * from `".$reciever_id."`  WHERE transactionid='$r_last_tid'";
    $result=mysql_query($sql) or die(mysql_error());
   ...
   //select the last transaction id in the sender's passbook
     $sql="SELECT MAX(transactionid) from `".$sender_id."`";
     $result=mysql_query($sql) or die(mysql_error());
     $rws=  mysql_fetch_array($result);
     $s_last_tid=$rws[0];
    
    //select the details in the last row of sender's passbook.
    $sql="SELECT * from `".$sender_id."` WHERE transactionid='$s_last_tid'";
    $result=mysql_query($sql) or die(mysql_error());
    while($rws= mysql_fetch_array($result)) {
    $s_amount=$rws[5];
    $s_name=$rws[2];
    ...
    elseif($s_total<500)
    {
        ...
        $sql1="insert into `".$reciever_id."` values('','$date','$r_name','$t_amount','0','$r_total','BY $s_name')";
        mysql_query($sql1) or die(mysql_error());
        
        //insert statement into sender passbook.
        $s_total=$s_amount-$t_amount;
        $sql2="insert into `".$sender_id."` values('','$date','$s_name','0','$t_amount','$s_total','TO $r_name')";
